document.addEventListener("DOMContentLoaded", (event) => {
  ...
  document
    .getElementById("view-active-quiz-btn")
    .addEventListener("click", viewActiveQuiz);
  ...
  function createQuiz(event) {
    ...
    const questionsElements = document.querySelectorAll(
  ...
  function updateQuizList() {
    ...
    quizzes.forEach((quiz, index) => {
      ...
      const quizTitleElement = document.createElement("div");
      quizTitleElement.textContent = quiz.title;

      const quizButtonsElement = document.createElement("div");
      quizButtonsElement.className =
        "d-flex gap-2 flex-row align-items-center justify-content-start";
      ...
      quizElement.appendChild(quizTitleElement);
      quizElement.appendChild(quizButtonsElement);
      quizButtonsElement.appendChild(selectButton);
      quizButtonsElement.appendChild(editButton);
      quizButtonsElement.appendChild(deleteButton);
  ...
  function viewActiveQuiz() {
    const activeQuizDetailsContainer = document.getElementById(
      "active-quiz-details"
    );
  ...
  function editQuiz(index) {
    ...
    quizToEdit.questions.forEach((question, questionIndex) => {
      ...
      questionElement.innerHTML = `
            <label>Pergunta: </label>
            <input type="text" class="edit-question form-control edit-question mb-2" value="${
              question.questionText
            }" required>
            <label>Opções: </label>
            <input type="text" class="edit-options form-control edit-question mb-2" value="${question.options.join(
              ", "
            )}" required>
            <label>Resposta Correta: </label>
            <input type="text" class="edit-correct-answer form-control edit-question mb-2" value="${
    ...
    document
      ...
      .addEventListener("click", () => saveChanges(index));
  }

  // Função para adicionar uma nova pergunta na modal de edição
  document
    .getElementById("edit-add-question-btn")
  ...
  function saveChanges(index) {
    ...
    const editedQuestionsElements = document.querySelectorAll(
      "#edit-questions-container > div"
    );

    const editedQuestions = Array.from(editedQuestionsElements).map(
      (element) => {
        const questionText = element.querySelector(".edit-question").value;
        const options = element.querySelector(".edit-options").value.split(",");
        const correctAnswer = element.querySelector(
          ".edit-correct-answer"
        ).value;
  ...
  function deleteQuiz(quizIndex) {
    ...
    const confirmDelete = confirm(
      "Tem certeza de que deseja excluir este quiz? Esta ação não pode ser desfeita."
    );
    if (confirmDelete) {
...
function viewQuiz(quizIndex) {
  ...
  selectedQuiz.questions.forEach((question, index) => {
    ...
    quizContent +=
      "<div>Resposta Correta: " + question.correctAnswer + "</div>";
    quizContent += "<hr>";
  });
...
function viewActiveQuiz() {
  ...
  if (activeQuiz) {
    ...
    const activeQuizDetailsElement = document.getElementById(
      "active-quiz-details"
    );
